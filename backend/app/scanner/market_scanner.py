...
#     async def _get_candles_for_symbol(self, symbol: str, interval: str, min_candles: int = None):
#         """
#         Fetches candles, ensuring at least `min_candles` are available.
#         Cache is only considered valid if it both meets the length
#         requirement AND is time-fresh; otherwise a larger fetch is made.
#         """
#         exchange = self.client.exchange_name
#         needed = min_candles or settings.binance_klines_limit
#         request_limit = min(max(needed, settings.binance_klines_limit), 1000)

#         cached = candle_cache.get(exchange, symbol, interval)
#         if cached and len(cached) >= needed and candle_cache.is_fresh(exchange, symbol, interval):
#             return cached

#         try:
#             fresh = await self.client.get_klines(symbol=symbol, interval=interval, limit=request_limit)
#         except Exception:
#             return cached or []
...
#                 tf,
#                 settings.htf_confluence_lookback_days,
...
#                     *(
#                         self._get_candles_for_symbol(sym_info.symbol, tf, min_candles=needed_by_tf[tf])
#                         for tf in HTF_CONFLUENCE_TFS
#                     )
#                 )
#             candles_by_tf = dict(zip(HTF_CONFLUENCE_TFS, tf_results))
#             if any(len(candles_by_tf[tf]) < needed_by_tf[tf] for tf in HTF_CONFLUENCE_TFS):
#                 return None
#             candles_by_tf = {tf: candles_by_tf[tf][-needed_by_tf[tf]:] for tf in HTF_CONFLUENCE_TFS}
...
#                     self._get_candles_for_symbol(sym_info.symbol, ltf_interval, min_candles=ltf_needed),
#                     *(
#                         self._get_candles_for_symbol(sym_info.symbol, tf, min_candles=needed_by_tf[tf])
#                         for tf in HTF_CONFLUENCE_TFS
#                     ),
#                 )
#             ltf_candles, *htf_lists = results
#             if len(ltf_candles) < ltf_needed:
#                 return None

#             htf_candles_by_tf = dict(zip(HTF_CONFLUENCE_TFS, htf_lists))
#             if any(len(htf_candles_by_tf[tf]) < needed_by_tf[tf] for tf in HTF_CONFLUENCE_TFS):
#                 return None
#             htf_candles_by_tf = {tf: htf_candles_by_tf[tf][-needed_by_tf[tf]:] for tf in HTF_CONFLUENCE_TFS}

#             return (
#                 sym_info.symbol,
#                 [c.model_dump() for c in ltf_candles[-ltf_needed:]],
...
class MarketScanner:
    ...
    async def scan_htf_liquidity(self, interval: str = "1h", top_n: int = None) -> List[dict]:
        ...
        async def fetch(sym_info):
            async with semaphore:
                candles_by_tf = {}
                for tf in HTF_CONFLUENCE_TFS:
                    candles = await self._get_candles_for_symbol(sym_info.symbol, tf)
                    if len(candles) < 20:
                        return None
                    candles_by_tf[tf] = candles[-60:]
            return (sym_info.symbol, {tf: [c.model_dump() for c in cs] for tf, cs in candles_by_tf.items()})
    ...
    async def scan_ltf_liquidity(
        ...
        async def fetch(sym_info):
            async with semaphore:
                ltf_candles = await self._get_candles_for_symbol(sym_info.symbol, ltf_interval)
                ...
                for tf in HTF_CONFLUENCE_TFS:
                    ...
                    if len(candles) < 20:
                        return None
                    htf_candles_by_tf[tf] = candles[-60:]
